...
fn test_omp_parallel_nested() bool {
    ...
    omp.parallel(.{})
        .run(.{ .shared = .{&counter} }, struct {
        fn f(f_counter: *i32) void {
            omp.critical(.{})
                .run(.{f_counter}, struct {
            ...
            omp.parallel(.{})
                .run(.{ .shared = .{f_counter} }, struct {
                fn f(pf_counter: *i32) void {
                    omp.critical(.{})
                        .run(.{pf_counter}, struct {
...
fn test_omp_parallel_private() !bool {
    ...
    omp.parallel(.{})
        .run(.{ .shared = .{ &sum, &num_threads }, .private = .{&sum1} }, struct {
        fn f(f_sum: *u32, f_num_threads: *u32, f_sum1: *u32) void {
            f_sum1.* = 7;

            omp.loop(.{ .idx = u32 })
                .run(.{ .shared = .{f_sum1} }, 1, 1000, 1, struct {
            ...
            omp.critical(.{})
                .run(.{ f_sum, f_num_threads, f_sum1.* }, struct {
